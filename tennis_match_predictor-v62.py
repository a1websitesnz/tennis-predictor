...
def fetch_top_300_from_api():
    ...
    try:
        ...
        elif isinstance(data, dict) and "data" in data and "rankings" in data["data"]:
            players_raw = data["data"]["rankings"][:300]
        else:
            st.error("⚠️ Unexpected API format.")
            return []

        return [f"{p['lastname']} {p['firstname'][0]}." for p in players_raw]
...
filtered_top_players = sorted(set(manual_top_300_names) & set(df_players_clean))
