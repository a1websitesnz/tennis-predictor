...
def extract_top_300_names_from_official_pdf():
    ...
    for page in doc:
        ...
        for line in text.split("\n"):
            # Match lines like: "1 Djokovic N."
            match = re.match(r"\s*(\d+)\s+([A-Za-z\-'`]+)\s+([A-Z])\.\s*$", line)
            if match:
                last_name = match.group(2).strip()
                first_initial = match.group(3).strip()
...
for top_player in manual_top_300_names:
    try:
        last, initial_dot = top_player.rsplit(" ", 1)
        norm_key_pattern = normalize_for_matching(last + initial_dot.replace(".", ""))
        for norm_name, original in norm_map.items():
            if norm_name.startswith(norm_key_pattern):
