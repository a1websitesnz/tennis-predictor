...
def fetch_top_300_from_api():
    ...
    try:
        data = response.json()
        st.write("🔍 API response preview:", data)

        players_raw = []
        if isinstance(data, dict):
            if "data" in data and isinstance(data["data"], list):
                players_raw = data["data"]
            elif "data" in data and "rankings" in data["data"]:
                players_raw = data["data"]["rankings"]
            elif "response" in data:
                players_raw = data["response"]
            else:
                st.error("❌ API format not recognized in dict.")
                return []
        else:
            st.error("❌ Unexpected API type.")
            return []

        top_players = []
        for entry in players_raw:
            if "player" in entry and "name" in entry["player"]:
                full_name = entry["player"]["name"]
                parts = full_name.strip().split()
                if len(parts) >= 2:
                    first_initial = parts[0][0]
                    last_name = parts[-1]
                    formatted = f"{last_name} {first_initial}."
                    top_players.append(formatted)

        st.write("✅ Extracted players:", top_players[:25])
        return top_players[:300]
...
if not filtered_top_players:
    ...
    st.info(f"✅ {len(filtered_top_players)} active top 300 players matched to dataset.")

# 🔍 Debug unmatched names
if st.checkbox("🔍 Show unmatched top 300 names"):
    unmatched = set(normalized_top_300.keys()) - set(normalized_dataset_players.keys())
    matched = set(normalized_top_300.keys()) & set(normalized_dataset_players.keys())
    st.write("Top 300 names not matched in dataset:")
    st.write(sorted([normalized_top_300[k] for k in unmatched]))
    st.success("✅ Matched top 300 player names:")
    st.write(sorted([normalized_top_300[k] for k in matched]))
...
if st.button("Predict Winner"):
    ...
    fallback_reason = ""
    if match_data.empty:
        fallback_reason = "No past matches found between selected players on specified surface and level. Trying surface only."
    ...
    if match_data.empty:
        fallback_reason = "No surface-level matches either. Using Elo rating difference only."
        st.warning(fallback_reason)

        player1_elo = df[(df["winner_name"] == player1)]["winner_elo"].mean()
        player2_elo = df[(df["winner_name"] == player2)]["winner_elo"].mean()

        if pd.isna(player1_elo):
            player1_elo = df[(df["loser_name"] == player1)]["loser_elo"].mean()
        if pd.isna(player2_elo):
            player2_elo = df[(df["loser_name"] == player2)]["loser_elo"].mean()

        if pd.isna(player1_elo) or pd.isna(player2_elo):
            st.error("Elo ratings not available for one or both players.")
        else:
            elo_diff = player1_elo - player2_elo
            predicted_winner = player1 if elo_diff > 0 else player2
            st.markdown(f"### 🧠 Predicted Winner (Elo-based fallback): **{predicted_winner}**")
    else:
