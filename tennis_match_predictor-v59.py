...
def extract_top_300_names_from_official_pdf():
    ...
    for page in doc:
        ...
        for line in text.split("\n"):
            ...
            if match:
                rank, last_name, first_name, country = match.groups()
                full_name = f"{last_name} {first_name[0]}."
                top_300_names.append(full_name)
    ...
    doc.close()
    if len(top_300_names) < 10:
        # fallback hardcoded list if extraction fails
        return ["Djokovic N.", "Alcaraz C.", "Sinner J.", "Medvedev D.", "Zverev A.", "Rublev A.", "Tsitsipas S.", "Rune H.", "Ruud C.", "Hurkacz H."]
