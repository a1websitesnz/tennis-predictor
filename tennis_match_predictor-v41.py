...
if os.path.exists(ranking_file):
    ...
    player_id_name_map["short_name"] = player_id_name_map["last_name"].astype(str) + " " + player_id_name_map["first_name"].astype(str).str[0] + "."
    merged = pd.merge(rankings_df, player_id_name_map, on="player_id")
    current_players = set(merged["short_name"].dropna().astype(str).unique())
    top_300_names = list(merged.sort_values("rank")["short_name"].unique())
...
players = [p for p in all_players if p in top_300_names]
