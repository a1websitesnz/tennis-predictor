...
if os.path.exists(ranking_file):
    ...
    player_id_name_map["short_name"] = player_id_name_map["last_name"].astype(str).str.strip() + " " + player_id_name_map["first_name"].astype(str).str[0].str.upper() + "."
    merged = pd.merge(rankings_df, player_id_name_map, on="player_id")
    top_300_names = list(merged.sort_values("rank")["short_name"].dropna().astype(str).unique())
    current_players = set(top_300_names)
...
# Filter player names to current top 300
...
normalized_top_300 = set([normalize_name(p) for p in top_300_names])
players = [p for p in all_players if normalize_name(p) in normalized_top_300]
