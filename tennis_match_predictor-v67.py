    ...
    try:
        ...
        if isinstance(data, dict):
            if "data" in data and isinstance(data["data"], list):
                ...
                raise ValueError("API format not recognized")
        elif isinstance(data, list):
            players_raw = data
        else:
            raise ValueError("Unexpected API type")

        top_players = [f"{p['lastname']} {p['firstname'][0]}." for p in players_raw if 'firstname' in p and 'lastname' in p]
...
matched_keys = sorted(set(normalized_top_300.keys()) & set(normalized_dataset_players.keys()))
filtered_top_players = [normalized_dataset_players[k] for k in matched_keys]
...
# 🔍 Debug unmatched and matched names
if st.checkbox("🔍 Show unmatched top 300 names"):
    ...
    st.write("✅ Matched top 300 player names:")
    st.write([normalized_dataset_players[k] for k in matched_keys])
